 ...
 *
 * 和Filter的区别：
 * 1.HandlerInterceptor 是spring框架
 * 2.Filter 是JDK提供的规范；二者依赖也不一样；
 *3.先执行Filter，在执行另外一个；
 *
 * 使用： 1.可以同时配置多个；
 * 2.返回一个布尔类型；表示调用链是否往下执行；
 * 3.如果需要往下执行，就直接if（xxx） return true；
 * 4.如果不能继续访问，return false； if（XXX）返回响应给客户端 return false；
 *
 *
 *
 * StringBoot 中的StringMvc.会默认提供处理的错误，Controller，url位/error；
 */
public class LogInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        //登陆失败，不允许访问的业务;
        //前端跳转，后端返回json
        String path = request.getServletPath();//请求的服务路径
